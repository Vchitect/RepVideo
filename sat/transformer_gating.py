...
class CrossAttention(torch.nn.Module):
    ...
    def __init__(self, hidden_size, num_attention_heads, attention_dropout_prob, output_dropout_prob, init_method,
        ...
        self.dense = RowParallelLinear(
            ...
            hidden_size,
            input_is_parallel=True,
            init_method=output_layer_init_method, bias=bias, params_dtype=params_dtype, module=self, name="cache",skip_init=skip_init,
...
class MLP(torch.nn.Module):
    def __init__(self, hidden_size, output_dropout_prob, init_method, inner_hidden_size=None,
        ...
        if is_gated_mlp:
            ...
            params_dtype=params_dtype,
            module=self,
            name="cache_h_to_4h_gate",
            skip_init=skip_init,
            device=device
        )
        self.num_experts = num_experts
        for i in range(1, num_experts):
            self.register_module(f"cache_h_to_4h_{i}", ColumnParallelLinear(
                self.hidden_size,
                self.inner_hidden_size,
                gather_output=False,
                init_method=init_method,
                ...
                params_dtype=params_dtype,
                module=self,
                name=f"cache_h_to_4h_{i}",
                skip_init=skip_init,
                device=device
            ))
            # Project back to h.
            self.register_module(f"cache_4h_to_h_{i}", RowParallelLinear(
                self.inner_hidden_size,
                self.hidden_size,
                input_is_parallel=True,
                init_method=output_layer_init_method,
                ...
                params_dtype=params_dtype,
                module=self,
                name=f"cache_4h_to_h_{i}",
                skip_init=skip_init,
                device=device,
                final_bias=row_parallel_linear_final_bias
            ))
            if is_gated_mlp:
                self.register_module(f"cache_h_to_4h_gate_{i}", ColumnParallelLinear(
                self.hidden_size,
                self.inner_hidden_size,
                gather_output=False,
                init_method=init_method,
...
class BaseTransformer(torch.nn.Module):
    def __init__(self,
        ...
        self.learnable_gating = (torch.nn.Parameter(torch.full((num_layers,), 0.5)))
        # self.learnable_gating = [torch.nn.Parameter(torch.full((1,), 0.0001)) for _ in range(num_layers)]
        
        # self.gating_parameters = [g for g in self.learnable_gating_split]
    ...
    def forward(self, input_ids, position_ids, attention_mask, *,
        ...
        # sanity check
        # print(self.learnable_gating)
        gating_parameters = self.learnable_gating.unbind(dim=0)
        ...
        encoder_hidden_states_cache = [hidden_states]
        output_per_layers = []
        if self.checkpoint_activations:
            ...
            output_this_layer = []
            while l < num_layers:
                if len(encoder_hidden_states_cache) > 1:
                    encoder_hidden_states_cache_input = torch.stack(encoder_hidden_states_cache, dim=0)
                    encoder_hidden_states_cache_input = torch.mean(encoder_hidden_states_cache_input, dim=0)
                else:
                    encoder_hidden_states_cache_input = encoder_hidden_states_cache[0]
                hidden_states = (gating_parameters[l] ) * encoder_hidden_states_cache_input + (1 - gating_parameters[l] ) * hidden_states
                ...
                if l % 6 == 0:
                    ...
                    encoder_hidden_states_cache = encoder_hidden_states_cache + [hidden_states]
            ...
            for i, layer in enumerate(self.layers):
                if len(encoder_hidden_states_cache) > 1:
                    ...
                    encoder_hidden_states_cache_input = encoder_hidden_states_cache[0]
                hidden_states = (gating_parameters[i] ) * encoder_hidden_states_cache_input + (1 - gating_parameters[i]) * hidden_states
